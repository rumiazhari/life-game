...
(function(root){
  ...
  function normalizeCondition(person,condition,options){
    const info=root.ConditionRegistry.condition(condition.definitionId||condition.id)||root.ConditionRegistry.condition('chronic');
    const world=options&&options.world||options;
    condition.definitionId=info.id;condition.id=info.id;condition.severity=clamp(number(condition.severity,info.base),0,5);
    ...
    condition.source=String(condition.source||'unclassified');condition.exposureType=String(condition.exposureType||info.kind);condition.contagious=condition.contagious==null?!!info.contagious:!!condition.contagious;
    ...
    condition.incubationLeft=Math.max(0,number(condition.incubationLeft,condition.state==='latent'?info.incubation:0));condition.complications=Array.isArray(condition.complications)?condition.complications:[];
