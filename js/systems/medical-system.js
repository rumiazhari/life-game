...
(function(root){
  ...
  function reconcileInstanceIds(person,conditions,state){
    ...
    const used=new Set();conditions.forEach(condition=>{const match=pattern.exec(String(condition.instanceId||''));if(match&&!used.has(condition.instanceId)){used.add(condition.instanceId);return;}do{counter+=1;condition.instanceId='condition:'+key+':'+integer(condition.onsetYear)+':'+String(counter).padStart(4,'0');}while(used.has(condition.instanceId));used.add(condition.instanceId);});state.instanceCounter=counter;
  }
