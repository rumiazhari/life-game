...
function terminateSubjectEmployment(reason,year){
  if(typeof EmploymentSystem==='object'&&EmploymentSystem&&typeof EmploymentSystem.activeForPerson==='function'&&typeof EmploymentSystem.dismiss==='function'&&typeof World!=='undefined'&&World){
    const contract=EmploymentSystem.activeForPerson(World,'subject')[0]||null;
    if(contract){
      EmploymentSystem.dismiss(World,contract.id,reason,year==null?World.year:year,{subject:S});
      return contract;
    }
  }
  // Legacy fallback for states without a persistent contract/system.
  S.employmentContractId=null;S.jobTier=0;S.jobName='Unemployed';S.career=null;S.careerYears=0;
  return null;
}
...
const EVENTS=[
 ...
 /* ---- world-state events: the settlement, the employer, the household,
         and the file itself push back, so no two runs rhyme ---- */
 {id:'rationline',a:[16,110],cd:5,if:s=>s.assets<200,v:['A rumour moved down the queue faster than the queue itself: the flour allotment was cut again. Everyone kept their place anyway.','The ration board posted a notice with three stamps on it. Nobody needed to read past the first line. The line said less.'],
   fx:{happiness:-1}},
 {id:'landlordnozzle',a:[18,110],cd:7,if:s=>s.assets<0&&s.lifestyle.housing!=='none'&&s.age>=18,v:['The landlord’s man knocked twice, looked at the ceiling crack as if it owed him money, and left a card with red edges.','A note under the door, in pencil sharpened past politeness: REMINDER. The word carried its own arithmetic.'],
   fx:{happiness:-3,health:-1}},
 {id:'pawnring',a:[18,110],cd:9,if:s=>s.assets<60,v:['Subject pawned the good watch for the season and bought it a story about a repair shop. The clerk stamped the ticket like a small mercy.','The pawnshop window had a ring in it that had been through four owners in a year, by its own tags. Subject’s grandmother’s spoon made five transactions of its own today.'],
   fx:s=>({assets:RI(40,150),happiness:-1})},
 {id:'soupkindness',a:[10,110],cd:6,if:s=>s.lifestyle.food==='meager'||s.assets<30,v:['A woman ahead of Subject in the mission line turned around and pressed her second bread roll into their hands. She did not stay to be thanked.','The ladle-man, unprompted, gave Subject the thick end of the pot and pretended not to hear the thank-you.'],
   fx:{happiness:3,relations:1}},
 {id:'factorywhistle',a:[16,60],cd:6,if:s=>{try{return s.jobTier>0&&World.settlements[(s.location&&s.location.settlementId)||World.activeSettlementId].economy.employmentIndex<0.35;}catch(e){return false;}},v:['The whistle went at noon and nobody came back until the foreman walked the floor himself. Subject stood with the others, hands in pockets, learning the shape of a rumour.','Half the floor did not show. The other half worked twice as hard pretending not to notice. The foreman posted a notice nobody signed.'],
   fx:{happiness:-2},side:s=>{s.scrutiny=clamp(s.scrutiny+3,0,100);}},
 {id:'overtimeculture',a:[16,64],cd:6,if:s=>{try{var c=EmploymentSystem.activeForPerson(World,'subject')[0];return !!(c&&c.workplace&&c.workplace.stress>0.62);}catch(e){return false;}},v:['The section head started scheduling “voluntary” Saturdays. The quotation marks did the work of a contract clause.','Nobody left before the floor supervisor. Nobody left before the deputy. Subject learned to read the room’s exits by who was still in it.'],
   fx:{health:-3,happiness:-2}},
 {id:'employermedal',a:[16,65],cd:8,if:s=>{try{var c=EmploymentSystem.activeForPerson(World,'subject')[0];return !!(c&&c.performance>0.85);}catch(e){return false;}},v:['A printed commendation arrived through internal post, third copy, carbon-smudged. It was, the covering note insisted, an honour of the modest kind.','Subject’s name was read out at the Monday assembly, mispronounced, to mild applause. A small envelope accompanied the mispronunciation.'],
   fx:{happiness:4},side:s=>{s.assets+=80;}},
 {id:'employerwobble',a:[16,65],cd:6,if:s=>{try{var c=EmploymentSystem.activeForPerson(World,'subject')[0];var b=c&&BusinessSystem.get(World,c.businessId);return !!(b&&b.status==='struggling');}catch(e){return false;}},v:['Two delivery vans were sold off over the weekend. The remaining vans were repainted so nobody would count them.','The requisition forms got longer while the stationery got thinner. Everyone understood both ends of that exchange.'],
   fx:{happiness:-3}},
 {id:'dividendday',a:[30,78],cd:8,if:s=>s.assets>4000,v:['A quietly profitable year at the savings house arrived as a letter with a number in it larger than expected. Subject read it twice, then once more aloud.','The clerk at the counter counted out the interest in used notes, each one softened by other people’s years.'],
   fx:s=>({assets:RI(120,320),happiness:2})},
 {id:'gala',a:[25,72],cd:9,if:s=>s.assets>6000&&s.relations>55,v:['An invitation on heavy paper: the Bureau’s Autumn Concord Gala, dress “respectful.” Subject went, was seen, and was entered into three separate guest lists.','Subject attended the district benefactors’ dinner. The soup was thin and the speeches thick, in the correct proportions for the room.'],
   fx:{happiness:5,looks:1,relations:2,assets:-200}},
 {id:'spousesurprise',a:[20,80],cd:6,if:s=>s.married,v:['{partner} came home with two cinema tickets and an alibi neither of them needed. The evening behaved like the ones from before the paperwork.','Subject found the good coffee in the cupboard and a note in {partner}’s hand: FOR THE ONE WHO NEVER ASKS. The note was filed, mentally, forever.'],
   fx:{happiness:4},side:s=>{const p=activePartnerContact();if(p){p.mood=clamp(p.mood+10,0,100);syncPartnerMirror();}}},
 {id:'kidscrap',a:[22,75],cd:5,if:s=>s.kids>0,v:['The children presented a “play” in the kitchen, three acts, all of them about the cat. The cat declined to attend. The reviews within the household were glowing.','One of the children made Subject a card at school, folded eight times, spelling two words wrong in ink that never fully dried. It went into the good drawer.'],
   fx:{happiness:3},side:s=>{s.familyMood=clamp(s.familyMood+8,0,100);}},
 {id:'parentvisit',a:[20,50],cd:8,if:s=>((s.mother&&s.mother.alive&&!s.mother.estranged)||(s.father&&s.father.alive&&!s.father.estranged)),v:['Sunday dinner at the old table. The chairs still knew everyone’s weight. Subject left with jars, advice, and a colder platform waiting home.','{mother} sent a parcel: socks, hard sweets, a newspaper clipping about nothing. {father} had, the note said, supervised the string.'],
   fx:{happiness:3,relations:2},side:s=>{if(s.mother&&s.mother.alive&&!s.mother.estranged)s.mother.mood=clamp(s.mother.mood+8,0,100);if(s.father&&s.father.alive&&!s.father.estranged)s.father.mood=clamp(s.father.mood+8,0,100);}},
 {id:'neighbourfeud',a:[16,110],cd:8,v:['The neighbours’ argument about the hedge reached its third year and its loudest week. Subject’s kitchen window held a seat for every performance.','A dispute over a borrowed ladder escalated to a formal complaint, in triplicate, to an office that handles fences, feuds, and nothing else.'],
   fx:{happiness:-2}},
 {id:'foundmoney',a:[10,110],cd:9,v:['Subject found coins in the winter coat from two years ago, along with a tram ticket for a journey already forgotten.','A wallet, empty of identity and full of intent, changed Subject’s week at the precise moment weeks needed changing. Subject handed it in, mostly.'],
   fx:s=>({assets:RI(20,95),happiness:2})},
 {id:'checkpointqueues',a:[16,110],cd:6,if:s=>{try{return s.scrutiny>35||World.settlements[(s.location&&s.location.settlementId)||World.activeSettlementId].security.surveillance>0.6;}catch(e){return false;}},v:['The checkpoint added a second table. The second table added an hour. The hour came out of everyone’s lunch and nobody’s budget.','Papers were checked twice going in and once coming out, which told everyone exactly which direction the Bureau thought trouble travelled.'],
   fx:{happiness:-2}},
 {id:'blackout',a:[8,110],cd:7,if:s=>{try{return World.settlements[(s.location&&s.location.settlementId)||World.activeSettlementId].publicHealth.sanitation<0.48;}catch(e){return false;}},v:['The lights failed for a night and a day. The building organised candles, complaints, and a committee, in that order, within the hour.','The water ran brown from the taps until Thursday. The Bureau’s notice suggested boiling, patience, and confidence, in ascending order of difficulty.'],
   fx:{health:-2,happiness:-2}},
 {id:'cheapmarket',a:[16,110],cd:7,if:s=>{try{return World.settlements[(s.location&&s.location.settlementId)||World.activeSettlementId].economy.foodPriceIndex<0.82;}catch(e){return false;}},v:['A surplus year upcountry: the market stalls groaned, the prices knelt, and for one season eating well was merely a habit instead of a plan.','The co-op had eggs. Actual eggs, in quantity, unrationed. Strangers smiled at each other in the queue like conspirators.'],
   fx:{happiness:3}},
 {id:'holdletter',a:[18,110],cd:7,if:s=>s.holdMember,v:['A letter arrived with no return address, written in the fold’s plain careful hand. It asked after Subject’s health and answered questions Subject had not yet asked.','A matchbook with a date inked inside appeared in Subject’s coat pocket. The fold remembers birthdays, debts, and everyone’s turn.'],
   fx:{happiness:2},side:s=>{Hold.trust=clamp(Hold.trust+4,0,100);}},
 {id:'holdtipoff',a:[18,110],cd:8,if:s=>s.holdMember&&Hold.heat>40,v:['A warning travelled faster than the warrants could: routes moved, names slept elsewhere for a month, and the heat passed over the fold like weather.','Someone burned papers that no longer existed and stood visible in doorways on purpose. The watchers watched the watcher, and the fold breathed.'],
   fx:{happiness:-3},side:s=>{Hold.heat=clamp(Hold.heat-8,0,100);}},
 {id:'streetmusic',a:[12,110],cd:6,v:['A violinist played outside the tram stop, badly and often, until the stop loved him for it. Coins gathered like pigeons.','Someone with an accordion and a grudge against silence held court by the bakery. The bread line clapped along, off-duty for three minutes.'],
   fx:s=>({happiness:2,assets:chance(.3)?RI(5,25):0})},
 {id:'unrestgrumble',a:[16,110],cd:6,if:s=>{try{return World.settlements[(s.location&&s.location.settlementId)||World.activeSettlementId].security.unrest>0.55;}catch(e){return false;}},v:['Graffiti bloomed overnight on the ministry annexe, spelled correctly, which worried the readers more than the words did.','The trams stopped running after nine. Officially: scheduling. Unofficially: everyone had seen the barricade drills too.'],
   fx:{happiness:-3}},
];
...
const PURSUITS=[
 ...
 {id:'lookwork',cat:'work',icon:'🚪',cost:1,avail:s=>!s.eduStage&&s.age>=16&&s.age<65&&s.jailUntil<=s.age&&!s.jobName.startsWith('Pensioner'),
   ...
   apply:(S2,item)=>{
     if(typeof VacancySystem==='object'&&VacancySystem&&typeof VacancySystem.applyAndResolve==='function'&&typeof World!=='undefined'&&World){
       ...
       if(application&&application.status==='accepted'&&result.reason!=='duplicate_application'){
       ...
       if(reason==='duplicate_application'){
         const dupStatus=application&&application.status;
         if(dupStatus==='accepted') return{fx:{happiness:-1},text:'Subject already had an application on file for that opening — it has already been accepted.',reason};
         if(dupStatus==='rejected') return{fx:{happiness:-1},text:'Subject already had an application on file for that opening — the earlier application was already rejected.',reason};
         if(dupStatus==='withdrawn') return{fx:{happiness:-1},text:'Subject already had an application on file for that opening — the application is no longer active.',reason};
         return{fx:{happiness:-1},text:'Subject already had an application on file for that opening — it remains pending.',reason};
       }
 ...
 {id:'favor',cat:'rel',icon:'🤝',cost:1,avail:s=>!s.eduStage&&s.relations>=70&&s.age>=16&&s.age>=s.favorCool&&s.jailUntil<=s.age,note:()=>S.jobTier===0?`call in a favor · a friend knows a job`:`call in a favor · a friend knows a way`,
   apply:()=>{S.favorCool=S.age+4;
     if(S.jobTier===0){
       if(typeof VacancySystem==='object'&&VacancySystem&&typeof VacancySystem.playerPortalVacancies==='function'&&typeof VacancySystem.applyAndResolve==='function'&&typeof EmploymentSystem==='object'&&EmploymentSystem&&typeof World!=='undefined'&&World){
         ...
         if(!openings.length) return{fx:{happiness:1},text:'Subject called in a favor. The friend asked around, but nobody had a real opening this year.',reason:'no_opening'};
         const opening=openings[0];
         const result=VacancySystem.applyAndResolve(World,opening.vacancyId,'subject',{subject:S,year:World.year,kind:'new_hire'});
         const application=result.application;
         if(application&&application.status==='accepted'){
           const business=BusinessSystem&&typeof BusinessSystem.get==='function'?BusinessSystem.get(World,opening.businessId):null;
           return{fx:{happiness:3},text:'Subject called in a favor. A friend of a friend needed someone reliable — subject started as '+opening.occupationName+' at '+(business?business.name:opening.businessName||'a new employer')+' the following Monday.'};
         }
         const reason=result.reason||(application?application.reason:'unfilled');
         return{fx:{happiness:-1},text:'Subject called in a favor, but it did not come to anything this year.',reason};
       }
       // Legacy fallback only when no persistent vacancy/employment system exists.
...
const DECISIONS=[
 ...
 {id:'earlyret',cost:1,cat:'work',avail:s=>s.age>=55&&s.age<=64&&s.jobTier>0,note:()=>`retire early · +HAPPINESS, pension`,
   apply:()=>{
     const activeContract=(typeof EmploymentSystem==='object'&&EmploymentSystem&&typeof EmploymentSystem.activeForPerson==='function'&&typeof World!=='undefined'&&World)
       ?(EmploymentSystem.activeForPerson(World,'subject')[0]||null):null;
     const pensionTier=activeContract?activeContract.jobTier:S.jobTier;
     S.pensionBase=300+pensionTier*300;
     if(activeContract&&typeof EmploymentSystem.retire==='function'){
       EmploymentSystem.retire(World,activeContract.id,World.year,{subject:S,reason:'early_retirement',terminalJobName:'Pensioner (early)'});
     } else {
       // Legacy fallback only when no persistent contract/system exists.
       S.employmentContractId=null;S.jobTier=0;S.jobName='Pensioner (early)';S.career=null;S.careerYears=0;
     }
     return{fx:{happiness:6},text:'Subject retired early, at noon on a Tuesday, and never went back. The pension paperwork arrived eventually.'}}},
